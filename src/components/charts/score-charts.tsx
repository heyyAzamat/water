...
import type { TrendAnalysis, WaterQuality } from "@/types";
...
function ChartTooltip({
  ...
  return (
    <div className="rounded-xl border border-white/12 bg-ink-850/95 px-3 py-2 shadow-xl backdrop-blur-xl">
...
export function ScoreHistoryChart({
  ...
  const data = points.map((p) => ({
    label: p.label ?? formatDate(p.date),
    score: p.score,
  }));
  ...
  if (data.length === 0) {
    return (
      <div
        ...
        style={{ height }}
      >
        <p className="text-[13px] text-ink-500">No assessments yet.</p>
      </div>
    );
  }
...
export function IndicatorBars({
  ...
  indicators: Array<{ label: string; severity: number; note?: string; detected: boolean }>;
}) {
  if (indicators.length === 0) {
    return (
      <p className="text-[13px] text-ink-500">
        No indicator matrix was recorded for this assessment.
      </p>
    );
  }
